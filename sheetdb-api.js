...
async function getMemberByStudentId(studentId) {
  const url = sheetUrl(SHEETS.MEMBERS, `&รหัสนักเรียน=${encodeURIComponent(studentId)}`);
  const rows = await request(url);
  return rows[0] || null;
}

/**
 * เข้าสู่ระบบด้วยรหัสนักเรียน + รหัสผ่าน
 * คืนค่าข้อมูลสมาชิกถ้าถูกต้อง, คืนค่า null ถ้ารหัสนักเรียนหรือรหัสผ่านไม่ตรง
 */
async function loginMember(studentId, password) {
  const member = await getMemberByStudentId(studentId);
  if (!member) return null;
  if (String(member.รหัสผ่าน || "") !== String(password)) return null;
  return member;
}

/** ค้นหาสมาชิกด้วยรหัสนักเรียน (ใช้ตอนเข้าสู่ระบบ) */
async function findMemberByStudentId(studentId) {
  const url = sheetUrl(SHEETS.MEMBERS, `&รหัสนักเรียน=${encodeURIComponent(studentId)}`);
  const rows = await request(url);
  return rows[0] || null;
}
